...
public ResponseEntity<?> createUser(@RequestBody User user) {
    try {
        return ResponseEntity.ok(authService.createUserByAdmin(user));
    } catch (Exception e) {
        return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
    }
}
    ...
    public List<AuditLog> getAuditLogs(@PathVariable Long userId) {
        return authService.getAuditLogs(userId);
    }

    @PostMapping("/users/{userId}/points")
    public ResponseEntity<?> addStarPoints(@PathVariable Long userId, @RequestBody Map<String, Integer> body) {
        try {
            Integer points = body.get("points");
            if (points == null || points <= 0) {
                return ResponseEntity.badRequest().body(Map.of("error", "Points must be a positive integer"));
            }
            return ResponseEntity.ok(authService.addStarPoints(userId, points));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> redeemPoints(@PathVariable Long userId, @RequestBody Map<String, Integer> body) {
        try {
            Integer points = body.get("points");
            if (points == null || points <= 0) {
                return ResponseEntity.badRequest().body(Map.of("error", "Points must be a positive integer"));
            }
            return ResponseEntity.ok(authService.redeemStarPoints(userId, points));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getUserTier(@PathVariable Long userId) {
        return authService.getUserById(userId)
                .map(u -> ResponseEntity.ok(Map.of(
                        "userId", u.getUserId(),
                        "starPoints", u.getStarPoints() == null ? 0 : u.getStarPoints(),
                        "memberTier", u.getMemberTier() == null ? "BRONZE" : u.getMemberTier())))
                .orElse(ResponseEntity.notFound().build());
    }
}