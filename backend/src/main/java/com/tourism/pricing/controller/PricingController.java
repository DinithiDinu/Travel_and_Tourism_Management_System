...
public class PricingController {
    ...
    public List<CoinRedemption> getRedemptions(@PathVariable Long userId) {
        return service.getRedemptionsByUser(userId);
    }
    ...
    public TourPackage createPackage(@RequestBody TourPackage p) {
    ...
    public List<PackageOffer> getOffersByPackage(@PathVariable Long packageId) {
        return service.getOffersByPackage(packageId);
    }

    @PostMapping("/offers")
    public PackageOffer createOffer(@RequestBody PackageOffer o) {
        return service.saveOffer(o);
    }
    ...
    public DiscountCategory createDiscount(@RequestBody DiscountCategory d) {
    ...
    // ── Member Tier Offers ──
    @GetMapping("/tier-offers")
    public List<MemberTierOffer> getAllTierOffers() {
        return service.getAllTierOffers();
    }

    @GetMapping("/tier-offers/{tier}")
    public List<MemberTierOffer> getOffersForTier(@PathVariable String tier) {
        return service.getOffersForTier(tier.toUpperCase());
    }

    @PostMapping("/tier-offers")
    public MemberTierOffer createTierOffer(@RequestBody MemberTierOffer offer) {
        return service.saveTierOffer(offer);
    }

    @PutMapping("/tier-offers/{id}")
    public MemberTierOffer updateTierOffer(@PathVariable Long id, @RequestBody MemberTierOffer offer) {
        offer.setOfferId(id);
        return service.saveTierOffer(offer);
    }

    @DeleteMapping("/tier-offers/{id}")
    public ResponseEntity<?> deleteTierOffer(@PathVariable Long id) {
        service.deleteTierOffer(id);
        return ResponseEntity.ok(Map.of("message", "Tier offer deleted"));
    }

    // ── Saved Cards ──
    @GetMapping("/saved-cards/user/{userId}")
    public List<SavedCard> getSavedCards(@PathVariable Long userId) {
        return service.getSavedCardsByUser(userId);
    }

    @PostMapping("/saved-cards")
    public SavedCard addSavedCard(@RequestBody SavedCard card) {
        return service.saveCard(card);
    }

    @DeleteMapping("/saved-cards/{id}")
    public ResponseEntity<?> deleteSavedCard(@PathVariable Long id) {
        service.deleteSavedCard(id);
        return ResponseEntity.ok(Map.of("message", "Card removed"));
    }

    @PutMapping("/saved-cards/{id}/default")
    public ResponseEntity<?> setCardDefault(@PathVariable Long id, @RequestBody Map<String, Long> body) {
        Long userId = body.get("userId");
        SavedCard updated = service.setCardAsDefault(id, userId);
