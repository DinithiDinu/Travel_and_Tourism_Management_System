...
public class BookingController {
    ...
    public ResponseEntity<?> createHotelBooking(@RequestBody HotelBooking booking) {
        try {
            return ResponseEntity.ok(service.saveHotelBooking(booking));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }

    @GetMapping("/hotel-bookings/user/{userId}")
    public List<HotelBooking> getHotelBookingsByUser(@PathVariable Long userId) {
        return service.getHotelBookingsByUser(userId);
    }
}
