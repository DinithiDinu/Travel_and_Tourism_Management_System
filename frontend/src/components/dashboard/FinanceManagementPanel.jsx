...
const TABS = [
    ...
    { id: 'payments', label: '💳 System Revenue' },
...
const FinanceManagementPanel = () => {
    ...
    const urls = {
        ...
        payments: { list: '/pricing/payments', id: 'paymentId', readOnly: true },
    ...
    const renderRow = (item) => {
        ...
        return (
            <tr key={id}>
                ...
                {tab === 'payments' && <><td>U-{item.userId}</td><td className="fp-money-green">${Number(item.amount).toFixed(2)}</td><td>{item.paymentMethod}</td><td>{new Date(item.transactionDate).toLocaleDateString()}</td><td><span className={`fp-badge fp-badge-${item.paymentStatus?.toLowerCase()}`}>{item.paymentStatus}</span></td></>}
                ...
                <td>
                    <div className="fp-actions">
                        {!cfg.readOnly && tab !== 'reports' && <button className="fp-btn fp-btn-edit" onClick={() => openEdit(item)}>Edit</button>}
                        {!cfg.readOnly && <button className="fp-btn fp-btn-danger" onClick={() => setDeleteTarget(item)}>Delete</button>}
                    </div>
                </td>
            </tr>
        );
    ...
    const renderHeaders = () => {
        ...
        if (tab === 'payments') return <tr><th>ID</th><th>User</th><th>Amount</th><th>Method</th><th>Date</th><th>Status</th><th>Actions</th></tr>;
