...
const TravelerDestinationsPanel = ({ onExplore, savedDestinations = [], onToggleSave }) => {
    ...
    useEffect(() => {
        const fetchTrips = async () => {
            try {
                ...
                const published = (data || []).filter(t => t.tripStatus !== 'Draft' && t.tripStatus !== 'Archived');
                setTrips(published);
            } catch (err) {
    ...
    if (loading) {
        return <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>Loading available trips...</div>;
    }

    return (
        <div>
            <div className="panel-header">
                <h2 className="panel-title">Explore Available Trips</h2>
                <p style={{ color: '#64748b', marginTop: '5px' }}>
                    Discover and book exciting trips created by our platform.
                </p>
            </div>
            ...
            <div style={{
                ...
                {trips.length === 0 ? (
                    <p style={{ color: '#64748b', gridColumn: '1 / -1' }}>No trips available at the moment.</p>
                ) : trips.map((trip) => (
                    <div key={trip.id} style={{
                        ...
                        <div style={{ position: 'relative', height: '220px' }}>
                            ...
                            <button
                                ...
                                aria-label={savedDestinations.includes(trip.id) ? "Unsave trip" : "Save trip"}
                            >
                                {savedDestinations.includes(trip.id) ? (
